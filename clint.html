...
<body class="bg-[#0e1621] text-[#f5f5f5] h-[100dvh] flex flex-col font-sans relative overflow-hidden telegram-bg">
    ...
    <script type="module">
        ...
        let isCallActiveOrRinging = false;
        ...
        async function getMeteredIceServers() {
            ...
            try {
                ...
                if (data && data.iceServers) {
                    data.iceServers = data.iceServers.concat(extraStunServers);
                } else {
                    data = { iceServers: extraStunServers };
                }
        ...
        window.startCall = async function() {
            ...
            } catch (e) { alert("تعذر الوصول للميكروفون، يرجى السماح به من إعدادات المتصفح"); closeCallUI(); }
        };
        ...
        function listenForIncomingCalls(clientKey) {
            onValue(ref(db, `single_chat_room/${clientKey}/call`), async (snapshot) => {
                ...
                if (!data) { 
                    if (isCallActiveOrRinging) closeCallUI(); 
                    return; 
                }
                if (data.caller === 'employee' && !data.answer && !isCallActiveOrRinging) {
                    isCallActiveOrRinging = true;
        ...
        window.answerIncomingCall = async function() {
            ...
            try {
                ...
                startTimer();
            } catch (e) { alert("خطأ أثناء الرد على المكالمة"); closeCallUI(); }
        };
        ...
        async function createPeerConnection(roomKey) {
            ...
            peerConnection.onicecandidate = e => { 
                if (e.candidate) push(ref(db, `single_chat_room/${roomKey}/call/candidates`), e.candidate.toJSON()); 
            };
        ...
        function listenForCandidates(roomKey) {
            onValue(ref(db, `single_chat_room/${roomKey}/call/candidates`), snap => {
                snap.forEach(c => { 
                    if (peerConnection && c.val() && peerConnection.remoteDescription) {
        ...
        function showCallScreen(text, incoming) {
            ...
            if (incoming) {
                ...
                document.getElementById("speakerBtn").classList.add("hidden");
            } else {
                document.getElementById("answerCallBtn").classList.add("hidden");
                document.getElementById("muteBtn").classList.remove("hidden");
                document.getElementById("speakerBtn").classList.remove("hidden");
        ...
        window.toggleSpeaker = function() {
            ...
            document.getElementById("speakerIcon").className = isSpeakerOn ? "fa-solid fa-volume-high" : "fa-solid fa-volume-xmark text-red-400";
        };

        function escapeHtml(str) {
            return str ? str.toString().replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'#'+m.charCodeAt(0)+';'}[m])) : '';
        }
    </script>
</body>
</html>
