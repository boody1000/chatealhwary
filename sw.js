...
// استقبال أمر عرض الإشعار الجاي مباشرة من صفحة العميل (postMessage)
// ده هو المسار الفعلي المستخدم دلوقتي طالما التاب مفتوح (حتى لو في الخلفية)
self.addEventListener('message', (event) => {
    const data = event.data;
    if (!data || data.type !== 'SHOW_NOTIFICATION') return;

    const notificationTitle = data.title || 'رسالة جديدة - الهواري للزواج';
    const notificationOptions = {
